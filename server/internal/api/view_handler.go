...
func fixXcomLayout(html string) string {
	// X.com (Twitter) 的虚拟滚动机制导致归档页面出现大片空白
	// 原因：
	// 1. 虚拟滚动容器有 min-height: 17213px（总内容高度）
	// 2. 推文使用 position: absolute + transform: translateY(9938px) 定位
	// 3. 捕获时用户已滚动，所以第一个可见推文被定位在很下方
	// 解决：移除 min-height 和 transform，让推文自然排列
	xcomCSS := `<style>
	/* 修复 X.com 虚拟滚动：移除容器的 min-height，让内容自然排列 */
	[aria-label*="时间线"] > div[style*="min-height"],
	[aria-label*="Timeline"] > div[style*="min-height"] {
		min-height: 0 !important;
	}
	/* 移除推文的绝对定位和 transform，让它们自然堆叠 */
...
func fixMTeamLayout(html string) string {
	// 移除 ant-image-error 类（捕获时图片未加载完成被 JS 标记为错误状态）
	antImageErrorRe := regexp.MustCompile(`(class="[^"]*)\bant-image-error\b([^"]*")`)
	html = antImageErrorRe.ReplaceAllString(html, "${1}${2}")

	// 隐藏 ant-image-mask（图片预览遮罩层，需要 JS 交互，归档页面不需要）
	// 修复 #app-content 的 SPA 布局（height:100% + overflow:hidden 在无 JS 时导致内容截断）
	mteamCSS := `<style>
	.ant-image-mask { display: none !important; }
	#app-content { align-items: stretch !important; height: auto !important; overflow: visible !important; }
</style>`
